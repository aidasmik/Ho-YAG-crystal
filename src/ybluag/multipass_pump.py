...
from .native_pump import transport_fixed_temperature


def pump_backend_name():
    """Report the active fixed-temperature pump implementation."""
    if (
        transport_fixed_temperature is not None
        and os.environ.get("YB_PUMP_BACKEND") != "python"
    ):
        return "numba_native"
    return "numpy_python"
...
def transport_multipass_pump(
    material: YbLuAGMaterial,
    pump_in_W_m2,
    density_scale_by_slice,
    thickness_m: float,
    passes: int,
    excited_fraction_by_slice,
    *,
    ...
    if (
        scale.ndim < 1
        or beta.shape != scale.shape
        or pump.shape != scale.shape[1:]
        or np.any(~np.isfinite(scale))
        or np.any(scale < 0)
        or np.any(~np.isfinite(beta))
        or np.any((beta < 0) | (beta > 1))
        or np.any(~np.isfinite(pump))
        or np.any(pump < 0)
        or not np.isfinite(thickness_m)
        or thickness_m <= 0
        or isinstance(passes, bool)
        or not isinstance(passes, int)
        or passes < 1
        or not np.isfinite(relay_efficiency)
        or not 0 < relay_efficiency <= 1
    ):
        raise ValueError("invalid multipass pump geometry or state")
    temperature = (
        None
        if temperature_K_by_slice is None
        else np.broadcast_to(
            np.asarray(temperature_K_by_slice, dtype=float), scale.shape
        )
    )
    if (
        temperature is None
        and pump.ndim == 2
        and scale.ndim == 3
        and pump_backend_name() == "numba_native"
    ):
        sigma_abs, sigma_em = material.cross_sections_m2(material.pump_wavelength_nm)
        return transport_fixed_temperature(
            pump,
            scale,
            beta,
            thickness_m,
    ...
    for ipass in range(passes):
        indices = (
            range(scale.shape[0])
            if ipass % 2 == 0
            else range(scale.shape[0] - 1, -1, -1)
        )
        for iz in indices:
            ...
            total_midpoint[iz] += exponential_cell_average(
                current, -alpha * scale[iz] * dz
            )
...
def steady_multipass_pump(
    ...
    *,
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        target = material.excited_fraction_cw(
            midpoint, temperature_K=temperature_K_by_slice
        )
